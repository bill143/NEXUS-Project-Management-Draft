...
def generate_demo_ifc(
    ...
    lines.append(f"FILE_NAME('{project_name}.ifc','{datetime.now(UTC).strftime('%Y-%m-%d')}',('NEXUS'),('DDC'),'','NEXUS v1.5.0','');")
    ...
    lines.append(f"#{org_id}= IFCORGANIZATION($,'NEXUS',$,$,$);")
    lines.append(f"#{po_id}= IFCPERSONANDORGANIZATION(#{person_id},#{org_id},$);")
    lines.append(f"#{app_id}= IFCAPPLICATION(#{org_id},'1.5.0','NEXUS','OE');")

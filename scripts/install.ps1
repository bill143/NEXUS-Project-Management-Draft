# NEXUS — One-Line Installer for Windows
#
# Usage:
...
$OE_INSTALL_DIR = if ($env:OE_INSTALL_DIR) { $env:OE_INSTALL_DIR } else { "$env:LOCALAPPDATA\NEXUS" }
...
function Install-Docker {
    ...
    if ($healthy) {
        Write-Ok "NEXUS is running at http://localhost:$OE_PORT"
    } else {
...
function Install-Uv {
    ...
    Invoke-Native -Description "uv tool install nexus" -Block {
        & $uvPath tool install nexus
    } | Out-Null
    Write-Ok "NEXUS installed!"
    Write-Host ""
    Write-Host "Run: nexus serve --port $OE_PORT --open"
}

function Install-Pip {
    Write-Host ""
    Write-Host "  +-------------------------------------------------+"
    Write-Host "  |  Installing NEXUS (pip mode)      |"
    Write-Host "  +-------------------------------------------------+"
    Write-Host ""
    ...
    Write-Info "[3/5] Installing nexus from PyPI..."
    ...
    Invoke-Native -Description "pip install nexus" -Block {
        & "$OE_INSTALL_DIR\venv\Scripts\python.exe" -m pip install --quiet --upgrade nexus
    } | Out-Null
    Write-Ok "[3/5] Package installed"

    # 4. Initialise database
    Write-Info "[4/5] Initialising local database..."
    $cliExe = if (Test-Path "$OE_INSTALL_DIR\venv\Scripts\nexus.exe") {
        "$OE_INSTALL_DIR\venv\Scripts\nexus.exe"
    } else {
    ...
    @"
@echo off
REM NEXUS launcher (auto-generated by install.ps1)
